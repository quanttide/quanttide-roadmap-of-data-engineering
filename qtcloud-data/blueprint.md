# Blueprint 治理设计（src/spec/blueprint.rs）

> 更新日期：2026-07-17 | 对应代码：`src/spec/blueprint.rs`（查看命令）+ `design`/`review`/`version`（生命周期）
> 注意：本文件为旧框架下的设计（GHTorrent 脱敏交付复盘），四层框架已替代框架结构（见 [index.md](index.md)），保留作为设计演进记录与治理愿景。
